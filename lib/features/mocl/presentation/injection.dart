...
abstract class ParserModule {
  ...
  (BaseParser, BaseApi) get _clienParser =>
      (const ClienParser(), ClienApi(Dio(), userAgentPc));

  @lazySingleton
  (BaseParser, BaseApi) get _meecoParser =>
      (const MeecoParser(false), MeecoApi(Dio(), userAgentMobile));

  @lazySingleton
  (BaseParser, BaseApi) get _damoangParser =>
      (const DamoangParser(false), DamoangApi(Dio(), userAgentMobile));

  @lazySingleton
  (BaseParser, BaseApi) get _naverCafeParser =>
      (const NaverCafeParser(), NaverCafeApi(Dio(), userAgentMobile));

  @lazySingleton
  (BaseParser, BaseApi) get _redditParser =>
      (const RedditParser(), RedditApi(Dio(), userAgentPc));
}
