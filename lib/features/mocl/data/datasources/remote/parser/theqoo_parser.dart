...
class TheQooPaser extends BaseParser {
  const TheQooPaser();
  ...
  Future<Result<List<ListItem>>> list(
    ...
    try {
      await Isolate.spawn(
          _parseListInIsolate,
          IsolateMessage<String>(receivePort.sendPort, response.data,
              lastId.intId, boardTitle, baseUrl, false));
  ...
  Future<Result<List<MainItem>>> main(Response response) async {
    ...
    if (container == null) {
      return Result.failure(GetMainFailure(message: 'Container is null'));
    }
