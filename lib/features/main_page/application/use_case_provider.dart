import 'package:mocl_flutter/core/application/app_provider.dart';
import 'package:mocl_flutter/features/main_page/application/repository_provider.dart';
import 'package:mocl_flutter/features/main_page/domain/usecases/get_main_list.dart';
import 'package:mocl_flutter/features/main_page/domain/usecases/set_main_list.dart';
import 'package:riverpod_annotation/riverpod_annotation.dart';

part 'use_case_provider.g.dart';

@riverpod
GetMainList getMainList(Ref ref) {
  final siteType = ref.watch(currentSiteTypeProvider);
  final repository = ref.watch(mainRepositoryProvider(siteType));
  return GetMainList(mainRepository: repository);
}

@riverpod
SetMainList setMainList(Ref ref) {
  final siteType = ref.watch(currentSiteTypeProvider);
  final repository = ref.watch(mainRepositoryProvider(siteType));
  return SetMainList(mainRepository: repository);
}
